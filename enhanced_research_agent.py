...
class EnhancedResearchAgent:
    ...
    async def _generate_validated_synthesis(self, 
        ...
        # First, check if sources are relevant to the query
        query_keywords = set(query.lower().split())
        relevant_sources = []
        irrelevant_sources = []
        
        for source in sources:
            title = source.get("metadata", {}).get("title", source.get("title", "")).lower()
            abstract = source.get("content", "").lower()
            
            # Check if source contains query-relevant content
            relevance_score = 0
        ...
        # If most sources are irrelevant, provide a different response
        if len(relevant_sources) < len(sources) * 0.5:
            logger.warning(f"Most sources ({len(irrelevant_sources)}/{len(sources)}) are irrelevant to query: {query}")
            return self._create_limited_relevance_synthesis(query, relevant_sources, irrelevant_sources)
...
6. If sources don't directly address the research question, clearly state this limitation
7. Focus on what the sources DO contain rather than what they don't
...
IMPORTANT: If the provided sources do not directly address "{query}", you must:
1. Clearly state that the sources found do not directly address the research question
2. Summarize what the sources actually discuss instead
3. Note the gap between what was asked and what was found
4. Suggest what types of sources would be needed to properly answer the question

Please provide a synthesis that:
1. Honestly assesses whether the sources address the research question
2. Uses proper in-text citations [Source X] for every claim
3. Acknowledges when sources are off-topic or irrelevant
4. Provides useful information from what is available
5. Clearly identifies limitations in the source selection

Remember: Intellectual honesty is more important than forcing an answer from irrelevant sources."""
    ...
    def _create_limited_relevance_synthesis(self, query: str, relevant_sources: List[Dict[str, Any]], irrelevant_sources: List[Dict[str, Any]]) -> str:
        """Create synthesis when most sources are irrelevant to the query."""
        synthesis_parts = [
            f"Research Analysis for: {query}",
            f"⚠️ **Source Relevance Issue Detected**\n"
        ]
        
        if relevant_sources:
            synthesis_parts.append(f"Found {len(relevant_sources)} sources that partially address the query:")
            for source in relevant_sources:
                title = source.get("metadata", {}).get("title", source.get("title", "Unknown"))
                citation_num = source.get("citation_number", 1)
                content = source.get("content", "")[:200] + "..."
                synthesis_parts.append(f"• {title} [Source {citation_num}]: {content}")
        else:
            synthesis_parts.append("**No sources directly address the research question.**")
        
        if irrelevant_sources:
            synthesis_parts.append(f"\n{len(irrelevant_sources)} sources were found but do not directly address '{query}':")
            for source in irrelevant_sources[:3]:  # Show first 3 irrelevant sources
                title = source.get("metadata", {}).get("title", source.get("title", "Unknown"))
                citation_num = source.get("citation_number", 1)
                synthesis_parts.append(f"• {title} [Source {citation_num}] - covers different topics")
        
        synthesis_parts.extend([
            "\n**Recommendations for Better Results:**",
            "1. Try more specific search terms related to your exact question",
            "2. Use technical terminology if applicable",
            "3. Consider alternative phrasings of your research question",
            "4. The search system may need refinement for this topic area"
        ])
        
        if relevant_sources:
            synthesis_parts.append(f"\n**Available Information:**\nWhile the sources don't fully address '{query}', they provide some related information as shown above.")
